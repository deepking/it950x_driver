...
#if 0
const double  Standard_CNRequiredValues[3][5] =    
{
      {5.1, 6.9, 7.9, 8.9, 9.7},       /** QPSK    CodeRate 1/2 ~ 7/8 CN Required Values*/
      {10.8, 13.1, 14.6, 15.6, 16.0},  /** 16QAM   CodeRate 1/2 ~ 7/8 CN Required Values*/
      {16.5, 18.7, 20.2, 21.6, 22.5},  /** 64QAM   CodeRate 1/2 ~ 7/8 CN Required Values*/
};/** NorDig 3.4.4.7 & Table 3.5*/


const double  Standard_HierarchicalCNRequiredValues[2][3][4] =    /**[16 or 64 QAM][CodeRate][a & H/L]   */
{
    {/**{HP(a=2),  HP(a=4),   LP(a=2), LP(a=4) } */
		{6.8, 5.8, 15, 19.5},		/** QPSK in 16QAM   CodeRate 1/2 CN Required Values*/
		{9.1, 7.9, 17.2, 21.4},		/** QPSK in 16QAM   CodeRate 2/3 CN Required Values*/
		{10.4, 9.1, 18.4, 22.5},	/** QPSK in 16QAM   CodeRate 3/4 CN Required Values*/
		
	},
	{/**{HP(a=1),  HP(a=2),   LP(a=1), LP(a=2) } */
		{10.9, 8.5, 16.7, 18.5},	/** QPSK in 64QAM   CodeRate 1/2 CN Required Values*/
		{14.1, 11, 19.1, 21.2},		/** QPSK in 64QAM   CodeRate 2/3 CN Required Values*/
		{15.7, 12.8, 20.9, 23.6},	/** QPSK in 64QAM   CodeRate 3/4 CN Required Values*/
	},
};/** NorDig 3.4.4.7 & Annex B-3, Tables 1 and 2.*/
#endif
/** local functions */
...
Dword Standard_getSignalQualityIndication (
	...
	error = Standard_getSNR (demodulator, &CNrec);
	if (error) goto exit;
	if(CNrec>15)
		CNrec = CNrec - 2;
	...
	if(channelModulation.hierarchy == 0){
		if(channelModulation.priority == Priority_HIGH)
		   CNrel = CNrec - Standard_CNRequiredValues[channelModulation.constellation][channelModulation.highCodeRate];
		else
		   CNrel = CNrec - Standard_CNRequiredValues[channelModulation.constellation][channelModulation.lowCodeRate];
	}else{//Hierarchical mode

		if(channelModulation.constellation == 1){ //16QAM

			if(channelModulation.priority == Priority_HIGH)
				CNrel = CNrec - Standard_HierarchicalCNRequiredValues[channelModulation.constellation][channelModulation.highCodeRate][channelModulation.hierarchy-2];
			else
				CNrel = CNrec - Standard_HierarchicalCNRequiredValues[channelModulation.constellation][channelModulation.lowCodeRate][channelModulation.hierarchy];

		}else if(channelModulation.constellation == 2){ //64QAM
			if(channelModulation.priority == Priority_HIGH)
				CNrel = CNrec - Standard_HierarchicalCNRequiredValues[channelModulation.constellation][channelModulation.highCodeRate][channelModulation.hierarchy-1];
			else
				CNrel = CNrec - Standard_HierarchicalCNRequiredValues[channelModulation.constellation][channelModulation.lowCodeRate][channelModulation.hierarchy+1];
	...
	if (error) {
		...
		BER = (double)postErrorCount/(double)postBitCount;	    
    }
		
	if(BER > 0.001)
		BER_SQI = 0;
	else if(BER <= 0.001 && BER > 0.0000001)
		BER_SQI = (Byte)(20*log10(1/BER)-40);
	else if(BER <= 0.0000001)
		BER_SQI = 100;


	if(CNrel<-7)
		SQI = 0;
	else if(-7 <= CNrel && CNrel < 3)
		SQI = (Byte)((((CNrel -3)/10) + 1) * BER_SQI);
	else if(CNrel >= 3)
		SQI = (Byte)BER_SQI;
