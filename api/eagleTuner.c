	...
	)
{
  // Using binary search to find the frequency interval in the table
    ...
    int outdPhi;
	int diff;
    while( ! ( (fIn - modulator->calibrationInfo.ptrIQtableEx[idx].frequency) >= 0 && (fIn - modulator->calibrationInfo.ptrIQtableEx[idx+1].frequency < 0) ) ){
	  if((fIn - modulator->calibrationInfo.ptrIQtableEx[idx].frequency)==0)
		  break;
      preIdx = idx;
      if(fIn - modulator->calibrationInfo.ptrIQtableEx[idx+1].frequency >= 0){
        idx = (preIdx + upper)/2;
        lower = preIdx;
      }else if(fIn - modulator->calibrationInfo.ptrIQtableEx[idx].frequency < 0){
        idx = (preIdx + lower)/2;
        upper = preIdx;
      }
  ...
  // Perform linear interpolation
    outdAmp = modulator->calibrationInfo.ptrIQtableEx[idx].dAmp + (fIn - modulator->calibrationInfo.ptrIQtableEx[idx].frequency) 
	...
	outdPhi = modulator->calibrationInfo.ptrIQtableEx[idx].dPhi + (fIn - modulator->calibrationInfo.ptrIQtableEx[idx].frequency) * diff / (modulator->calibrationInfo.ptrIQtableEx[idx+1].frequency - modulator->calibrationInfo.ptrIQtableEx[idx].frequency);
	if (outdPhi>=16384) {
...
Dword EagleTuner_setIQCalibration(
    ...
    interpolation(modulator, frequency, ptrdAmp, ptrdPhi);
