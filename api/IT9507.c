...
Dword IT9507_selectBandwidth (
	...
	switch (bandwidth) {
		...
		case 1000:              /** 1M */
			...
			temp4 = 0x03;	//0xD814
			temp5 = 0x00;	//0xF741
			error = IT9507_writeRegister (modulator, Processor_LINK, 0xD812, 4);
			if (error) goto exit;
			break;
...
Dword IT9507_setFrequency (
	...
	Byte TABLE_NROW = modulator->calibrationInfo.tableGroups;
...
Dword IT9507_initialize (
	...
	error = IT9507_readRegisters (modulator, Processor_OFDM, 0xf752, 2, c1_default_value);
	if (error) goto exit;
	error = IT9507_readRegisters (modulator, Processor_OFDM, 0xf754, 2, c2_default_value);
	if (error) goto exit;
	error = IT9507_readRegisters (modulator, Processor_OFDM, 0xf756, 2, c3_default_value);
	if (error) goto exit;
...
Dword IT9507_resetPSBBuffer (
	...
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xF9CC, 0);
	if (error) goto exit;
	...
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xF9CC, 1);
...
Dword IT9507_adjustOutputGain (
	...
	*gain = i/10;
	...
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf752, (Byte)(c1value&0x00ff));
	if (error) goto exit;		
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf753, (Byte)(c1value>>8));
	if (error) goto exit;		
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf754, (Byte)(c2value&0x00ff));
	if (error) goto exit;
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf755, (Byte)(c2value>>8));
	if (error) goto exit;
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf756, (Byte)(c3value&0x00ff));
	if (error) goto exit;
	error = IT9507_writeRegister (modulator, Processor_OFDM, 0xf757, (Byte)(c3value>>8));
	if (error) goto exit;
...
Dword IT9507_suspendMode (
	...
	Byte temp;
	error = IT9507_readRegister (modulator, Processor_OFDM, 0xFB24, &temp);//get power setting
	...
	if(error == ModulatorError_NO_ERROR){
		...
		error = IT9507_writeRegister (modulator, Processor_OFDM, 0xFB24, temp);
	}
	return (error);
}
